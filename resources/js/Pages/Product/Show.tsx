...
function Show({product, variationOptions}: {product: Product, variationOptions: number[]}) {
  ...
  const computedProduct = useMemo(()=> {
    ...
    for (let variation of product.variations) {
      const optionsIds = variation.variation_type_options_id.sort();
      if (arraysAreEqual(selectedOptionIds, optionsIds)) {
       return {
  ...
  function chooseOption(typeId: number, option: VariationTypeOption, updateRouter: boolean = true) {
  ...
  useEffect(() => {
    for (let type of product.variationTypes) {
      ...
      chooseOption(type.id, type.options.find(op => op.id == selectedOptionId) || type.options[0], false);
    }
  }, []);
          ...
          {type.type === 'radio' && (
            <div className={"flex join mb-4"}>
              {type.options.map(option => (
                <input onChange={()=>chooseOption(type.id,option)} key={option.id} className={"join-item btn"} type={"radio"} value={option.id}
                       checked={selectedOptions[type.id]?.id == option.id}
                name={'variation_type_' + type.id} aria-label={option.name}/>
              ))}
            </div>
          )}
        </div>
